...
/**
 * Normalizes every product image response shape used by the storefront.
 * The database API returns `images`, while legacy products may still expose
 * `imageUrls` or a single `imageUrl`.
 */
export function getProductImages(product) {
  ...
  const images = [
    ...
    ...(Array.isArray(product.imageUrls)
      ? product.imageUrls.map((url) => ({ url, alt: '' }))
      : []),
