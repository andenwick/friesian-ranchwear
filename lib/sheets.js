 ...
 * Uses a 5-minute cache to minimize API calls.
 *
 * Sheet columns: name, price, imageUrl, active, category, display, sizes, description, features
 */
export async function getProductsFromSheet() {
  ...
  // Fetch fresh data from Google Sheets (columns A through I)
  const response = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
    range: 'Products!A:I',
  });
  ...
  const products = rows
    ...
    .map((row, index) => {
      const imageUrls = row[2] ? row[2].split(',').map(url => url.trim()).filter(Boolean) : [];
      return {
        id: index + 2, // Row number (1-indexed, accounting for header)
        name: row[0] || '',
        price: row[1] || '',
        imageUrl: imageUrls[0] || '', // Primary image for cards
        imageUrls: imageUrls, // All images for detail page
        active: row[3] || '',
        category: row[4] || '',
        display: row[5] || 'Both', // Default to "Both" if not specified
        sizes: row[6] ? row[6].split(',').map(s => s.trim()) : [],
        description: row[7] || '',
        features: row[8] ? row[8].split('|').map(f => f.trim()).filter(Boolean) : [],
      };
    })
