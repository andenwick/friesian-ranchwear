  ...
  switch (action.type) {
    case ACTIONS.ADD_ITEM: {
      ...
      if (existingIndex > -1) {
        ...
        const newImage = product.images?.[0]?.url || product.imageUrls?.[0] || product.image || product.imageUrl || null;
        newItems[existingIndex] = {
          ...newItems[existingIndex],
          quantity: newItems[existingIndex].quantity + quantity,
          // Update image if we have one (fixes items added before image fix)
          image: newImage || newItems[existingIndex].image,
        };
        return { ...state, items: newItems };
      }

      // Add new item
      // Parse price - handle string prices like "$49.99" or numeric prices
      const rawPrice = variant?.price || product.basePrice || product.price;
      ...
      const newItem = {
        ...
        image: product.images?.[0]?.url || product.imageUrls?.[0] || product.image || product.imageUrl || null,
        quantity,
      };
